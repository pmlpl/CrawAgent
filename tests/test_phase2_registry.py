...
def test_discover_tools_finds_core_count():
    """discover_tools 应发现 21 个内置工具；插件工具另计（P2-9 插件规范）。"""
    # 清缓存，强制重新扫描
    import crawagent.tools.registry as reg
    reg._DISCOVERY_CACHE = None
    ...
    # 内置（源码在 crawagent/ 下）= 21；插件（source_file 以 plugins/ 开头）另计
    builtin = [s for s in specs if not s.source_file.startswith("plugins")]
    assert len(builtin) == 21, f"内置工具应为 21 个，实际 {len(builtin)}: {[s.name for s in builtin]}"
