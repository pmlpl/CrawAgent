...
def test_snapshot_masks_api_key(client):
    ...
    settings_mod.core.ENV_FILE.write_text("LANGSMITH_API_KEY=sk-real-secret-1234\n", encoding="utf-8")
    data = client.get("/api/settings").json()
...
def test_save_langsmith_masked_key_skipped(client):
    ...
    settings_mod.core.ENV_FILE.write_text("LANGSMITH_API_KEY=sk-keep-me-9999\n", encoding="utf-8")
    r = client.post("/api/settings", json={
...
def test_save_langsmith_empty_key_does_not_blank(client):
    ...
    settings_mod.core.ENV_FILE.write_text("LANGSMITH_API_KEY=sk-existing-aaaa\n", encoding="utf-8")
